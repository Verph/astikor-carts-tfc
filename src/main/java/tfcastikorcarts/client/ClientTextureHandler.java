...
public class ClientTextureHandler implements Initializer
{
    @Override
        ...
        for (Wood wood : Wood.VALUES)
        {
            new AssembledTextureFactory()
                .add(AstikorHelpers.identifier("textures/entity/animal_cart/" + wood.name().toLowerCase(Locale.ROOT) + ".png"), new AssembledTexture(64, 64)
                    .add(new Material(TFCBlocks.WOODS.get(wood).get(Wood.BlockType.PLANKS).getId(), 16)
                        .fill(0, 0, 60, 38, Material.R0, 0, 2)
                        .fill(0, 28, 20, 33, Material.R90, 4, -2)
                        .fill(12, 30, 8, 31, Material.R270, 0, 4)
                    )
                    .add(new Material(TFCBlocks.WOODS.get(wood).get(Wood.BlockType.STRIPPED_LOG).getId(), 16)
                        .fill(54, 54, 10, 10, Material.R0, 0, 2)
                    )
                    .add(new Material(TFCBlocks.WOODS.get(wood).get(Wood.BlockType.LOG).getId(), 16)
                        .fill(0, 21, 60, 4, Material.R90)
                        .fill(46, 60, 8, 4, Material.R90)
                    )
                    .add(new Material(TFCBlocks.ROCK_BLOCKS.get(Rock.DACITE).get(Rock.BlockType.RAW).getId(), 16)
                        .fill(62, 55, 2, 9)
                    )
                )
                ...
                .add(AstikorHelpers.identifier("textures/entity/supply_cart/" + wood.name().toLowerCase(Locale.ROOT) + ".png"), new AssembledTexture(64, 64)
                    ...
                    .add(new Material(TFCBlocks.SOIL.get(SoilBlockType.DIRT).get(SoilBlockType.Variant.LOAM).getId(), 16)
                        .fill(0, 45, 16, 17)
                    )
                );
        }
